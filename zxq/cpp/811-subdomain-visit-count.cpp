...
public:
    vector<string> subdomainVisits(vector<string>& cpdomains) {
        ...
        for (auto& cpdomain : cpdomains) {
            ...
            while (true) {
                if ((pos = domain.find('.')) == string::npos) {
                    break;
                }
        ...
        for (auto i = 0; i < domains.size(); ++i) {
            domains[i] = to_string(visits[i]) + " " + domains[i];
        }
        return domains;
    }
};
